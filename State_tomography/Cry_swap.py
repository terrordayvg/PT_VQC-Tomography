...
def adam2(thetas: np.ndarray, m: np.ndarray, v: np.ndarray, iteration: int,
    ...
    for i in range(0, num_thetas):
        m[i] = beta1 * m[i] + (1 - beta1) * grad_loss2[i]
        v[i] = beta2 * v[i] + (1 - beta2) * grad_loss2[i]**2
...
def grad_loss2(qc: QuantumCircuit,
   ...
   ## index_list = get_cry_index(thetas,depth,size)
    grad_loss2 = np.zeros(len(thetas))
	#print("Index_list")
	#print(index_list)
    #print(index_list)
    for i in range(0, len(thetas)):
        if index_list[i] == 0:
            grad_loss2[i] = single_2term_psr2(qc, thetas,size,depth,
                               i)
        if index_list[i] == 1:
            grad_loss2[i] = single_4term_psr2(qc, thetas,size,depth,
                               i)
    return grad_loss2
...
def Adam2_QST(depth, size, ites):
    ...
    for i in range(ites):
        ...
        thetas=adam2(thetas,s,v,ites,grad)
