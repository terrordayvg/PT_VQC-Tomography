...
size = 2 #Not an hypermparameter but needed to be defined here
depth = 3 #Not an hypermparameter but needed to be defined here
alpha = 0.001
b_1 = 0.9            
b_2 = 0.999
epsilon = 10 ** (-8)
...
def create__ansatz(qc: qiskit.QuantumCircuit,
	...
	for i in range(0, num_layers):
        ...
        #print(phis)
        #print("")
		qc = create_block(qc, phis[:size], phis[size:size * 2],size)
...
def circuit_update(thetas, depth, size):
	...
	qc = create__ansatz(qc, thetas, size,depth)
...
def grad_loss(qc: QuantumCircuit,
	...
	for gate in qc.data:#
		...
		if(str(gate[0].name)=="rx" or gate[0].name=="ry" or gate[0].name=="rz"):
			index_list[count]=0
			count=count+1
...
def plot_loss(loss,depth,size,ites):


	plt.plot(loss,label='Cost_Loss_function')
...
#Run the swap test
loss=Adam_QST(depth, size, ites)
#Loss_cry,Loss_cx=tomography.compare_W.Run_Vd_method(DataNum,Max_Tr,num_qubits,num_layers)
plot_loss(loss,depth,size,ites)